...
-- 3. Atlasīt tikai nosaukumu un populacijas kolonnas (SELECT)
-- 4. Atlasīt pilsētas kuras ir jaunākas par 200 gadiem (WHERE)
-- 5. Atlasīt pilsētas ar mazāk vai vienādas ar 5200 iedzīvotāju skaitu (WHERE)
-- 6. Atlasīt top 15 lielākas pilsētas pēc iedzīvotāju skaita (LIMIT, ORDER BY)
...
INSERT INTO Product(title, year, population, region) VALUES('Ādaži', 2022, 7482, 'Ādažu novads')
...
SELECT title, population FROM Product;

SELECT * FROM Product WHERE 2024-year<=200;

SELECT * FROM Product WHERE population<=5200;

SELECT * FROM Product ORDER BY population DESC LIMIT 15;

SELECT COUNT(title) AS pilsētu_skaitu FROM Product;

SELECT DISTINCT region FROM Product;

DELETE FROM Product WHERE title="Durbe";
SELECT * FROM Product;

SELECT COUNT(population)FROM Product GROUP BY region;
